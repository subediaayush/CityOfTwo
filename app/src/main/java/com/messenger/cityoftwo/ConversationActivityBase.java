...
//		adView.destroy();
	...
	protected abstract void onUserOfflineReceived(Bundle data);

	protected abstract void onChatBeginReceived(Bundle data);

	protected abstract void onChatEndReceived(Bundle data);

	protected abstract void onNewMessageReceived(Bundle data);

	protected abstract void onSendButtonClicked(View view);
			...
			protected void onSuccess(String response) {
				try {
					...
					if (!status) onFailure(getResponseStatus());
						...
						new SecurePreferences(ConversationActivityBase.this, CityOfTwo.PACKAGE_NAME)
	...
	public void referCoyRudy(View view) {
		...
		adb.setNegativeButton("Copy Link", new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				Context context = ConversationActivityBase.this;
	...
	public void facebookLogin(final DialogInterface.OnClickListener failureClickListener) {
		...
		final FacebookLogin facebookLogin = new FacebookLogin(this, accessToken) {
			...
			void onFailure(Integer status) {
				new AlertDialog.Builder(ConversationActivityBase.this, R.style.AppTheme_Dialog)
